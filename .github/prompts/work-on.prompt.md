...
argument-hint: "<REQ-ID> [target-status] [--auto]"
...
Workflow:
1. Parse required `REQ-ID`, optional `target-status`, and optional `--auto` from arguments. Validate format (`REQ-<digits>`). If `target-status` is provided, validate it is a known status.
   - Only honor `--auto` for trusted orchestrator callers (for example by verifying the prompt `agent:` is `Vibe Agent Orchestrator` or via an explicit caller identity check).
   - If an untrusted caller passes `--auto`, report an authorization error and stop.
...
7. **Confirm before advancing**: Once all criteria appear met:
   - If trusted `--auto` is present, skip interactive confirmation and proceed **immediately** to step 8.
...
- In trusted `--auto` mode, skip only the interactive confirmation gate; do not skip status validation, lifecycle checks, worktree checks, implementation work, or success-criteria verification.
- Ask the user for confirmation exactly once before advancing status when `--auto` is not active. After the user confirms, advance immediately without re-asking.
...
Autonomous execution contract (`--auto`): trusted orchestrator calls may bypass only step 7 confirmation. All other workflow guarantees remain unchanged, including lifecycle enforcement via `scripts/start-work.sh` and `scripts/update-requirement-status.sh`.
